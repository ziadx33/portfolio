...
export default function Form() {
	...
	async function contactFormSubmit(e: FormEvent) {
		e.preventDefault();
		if (
			nameErrors.some((e) => !e.success) ||
			emailErrors.some((e) => !e.success) ||
			subjectErrors.some((e) => !e.success) ||
			bodyErrors.some((e) => !e.success)
		) {
			setIsError(true);
			return;
		}
		setIsSending(true);
		setIsError(false);
		const res = await axios.post("https://formspree.io/f/mgegorre", {
			name,
			email,
			subject,
			body,
	...
	return (
		<form
			...
			className="lg:w-2/4 w-full h-full flex flex-col gap-1.5 items-center p-6"
		>
			<Input
				value={name}
				onChange={(e) => setName(e.target.value)}
				validations={nameErrors}
			...
			<Textarea
				...
				validations={bodyErrors}
				placeholder="How can I help?"
				name="body"
				className="min-h-36 max-h-72 mb-2.5"
			/>
