...
export default function Page() {
    ...
    const aboutObserver = useIntersectionObserver(aboutRef, { threshold: 0.2 });
    ...
    useEffect(() => {
        (async () => {
            ...
            const [{ data: github }, { data: codewars }] =
    ...
    return (
        <>
            <LoadingPage
                loadings={[
                    { name: "projects", status: isProjectsLoading },
                    { name: "stats", status: isStatsLoading },
                ]}
            />
            <Header />
            <Hero />
            <div ref={aboutRef} className="w-full min-h-screen">
                <About />
                <Projects />
                <Contact />
                <Footer />
            ...
            <Button
                ...
                className={`fixed bottom-12 right-16 z-30 transition-opacity opacity-0 ${isAboutVisable ? "opacity-100" : ""}`}
            >
                <ArrowUp />
            </Button>
        </>
